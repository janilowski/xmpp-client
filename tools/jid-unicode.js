...
function regex(name, values) {
  ...
  for (let cp = 0; cp < UNICODE_END; cp++) {
    ...
    ranges.push(escape(start) + (cp === start ? "" : `-${escape(cp)}`));
  }
  return `export const ${name} = /[${ranges.join("")}]/u;\n`;
}
...
output += regex("identifier", identifier);
output += regex("unassigned", unassigned);
output += regex(
  "freeform",
  opaque.map((value, cp) => (value && !identifier[cp] ? 1 : 0)),
);
output += regex(
  "idnaDisallowed",
  identifier.map((value, cp) => (value && !idna[cp] ? 1 : 0)),
);
...
for (const [name, file, values] of [
  ...
  output += regex(name, union(file, values));
}
...
output = await format(output, { parser: "babel" });
